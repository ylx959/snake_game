...
class PostgresLeaderboardRepository:
    ...
    def _create_schema(self) -> None:
        """Create the leaderboard table and indexes on first startup."""
    ...
    def record(
        ...
        """Store a player's best score.

        Returns True only when a new player is inserted or an existing
        player's score improves.
        """
        cleaned = clean_nickname(nickname)
        if cleaned.nickname is None:
            return False
        ...
        display = cleaned.nickname[:MAX_NICKNAME]
        key = display.casefold()
        timestamp = achieved_at if achieved_at is not None else time.time()
        ...
        with self._pool.connection() as connection:
            result = connection.execute(
                ...
                ON CONFLICT (nickname_key) DO UPDATE SET
                    ...
                    score       = EXCLUDED.score,
                    achieved_at = EXCLUDED.achieved_at
                WHERE EXCLUDED.score > solo_scores.score
                RETURNING id
                """,
                (display, key, score, timestamp),
            )
            return result.fetchone() is not None

    def top(self, limit: int = TOP_LIMIT) -> list[LeaderboardEntry]:
        """Return the highest scores, with earlier ties first."""
        limit = max(0, min(int(limit), TOP_LIMIT))
    ...
    def reserved_nicknames(self, limit: int = TOP_VISIBLE) -> set[str]:
        """Return the visible leaderboard names in case-folded form."""
        return {entry.nickname.casefold() for entry in self.top(limit)}

    def clear(self) -> None:
        """Delete all leaderboard entries."""
        with self._pool.connection() as connection:
            connection.execute("DELETE FROM solo_scores")
